...
PATH = Path(__file__).parents[1] / "local_modeling_adapter.py"
SPEC = importlib.util.spec_from_file_location("local_modeling_adapter", PATH)
...
class LocalModelingAdapterTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _mark_recording_ready(self, operation_id: str = "safe-point") -> None:
        adapter._atomic_json(
            adapter._ledger(self.repo, "run-local-123"),
            {
                ...
                "recording_grant": {
                    "harness_run_id": "harness-local-123",
                    "operation_id": operation_id,
                },
            },
        )
        adapter.smd.bind_local_execution(
            self.run_dir,
            build_session_id="session-1",
            harness_run_id="harness-local-123",
        )
    ...
    def test_start_then_commit_business_creates_once_and_binds_platform_cq(self) -> None:
        ...
        with mock.patch.object(adapter, "_request", side_effect=request):
            ...
            committed = adapter.commit_business(
                self.repo, self.run_dir, self.config, manifest, "safe-point"
            )
        self.assertEqual(started["references"]["build_session_id"], "session-1")
        self.assertEqual(started["next_action"], "activate-harness")
        ...
        task = adapter.smd._read_json(self.run_dir / "units/u/task.json")
        self.assertEqual(task["competency_question_ids"], ["remote-cq-1"])
        self.assertEqual(
            sum(
    ...
    def test_start_blocks_business_until_harness_health_is_recorded(self) -> None:
        ...
        with mock.patch.object(adapter, "_request", side_effect=request) as request_mock:
            started = adapter.start(self.repo, self.run_dir, self.config)
            with self.assertRaisesRegex(adapter.AdapterError, "recording_health_required"):
    ...
    def test_reconcile_uses_real_batch_detail_attempt_without_resubmission(self) -> None:
        ...
        with (
            ...
            mock.patch.object(
                ...
                return_value={
                    "run_id": "run-local-123",
                    "local_execution": {"harness_run_id": "harness-local-123"},
                },
            ),
    ...
    def test_safe_point_grants_are_required_by_each_protected_action_and_single_use(self) -> None:
        protected = (
            adapter.commit_business,
        ...
        for action in protected:
            self.assertIn("_consume_recording_grant", inspect.getsource(action))
        ...
        for invoke in invocations:
            with (
                ...
                mock.patch.object(
                    adapter,
                    "_consume_recording_grant",
                    side_effect=adapter.AdapterError("recording_health_required"),
                ),
                ...
                with self.assertRaisesRegex(adapter.AdapterError, "recording_health_required"):
                    invoke()
            request.assert_not_called()
        self._mark_recording_ready("grant-1")
        run = adapter.smd._load_run(self.run_dir)
        adapter._consume_recording_grant(self.repo, run, "grant-1")
        with self.assertRaisesRegex(adapter.AdapterError, "recording_health_required"):
            adapter._consume_recording_grant(self.repo, run, "grant-1")
        authorized = adapter.authorize_recording_unavailable(
            self.repo, self.run_dir, "exception-1", "user explicitly accepts incomplete recording"
        )
        self.assertEqual(authorized["status"], "ok")
        adapter._consume_recording_grant(self.repo, run, "exception-1")
        with self.assertRaisesRegex(adapter.AdapterError, "recording_health_required"):
            adapter._consume_recording_grant(self.repo, run, "exception-1")
    ...
    def test_apply_reuses_materialized_content_and_binds_without_token_leak(self) -> None:
        ...
        with (
            ...
            applied = adapter.apply_next(self.repo, self.run_dir, self.config, "o", "safe-point")
        payload = sent[0]
    ...
    def test_verify_executes_cq_lifecycle_and_invalid_evidence_is_rejected(self) -> None:
        ...
        with (
            ...
            result = adapter.verify(
                self.repo, self.run_dir, self.config, "o", verification, "safe-point"
            )
        self.assertEqual(result["status"], "ok")
    ...
    def _refresh_recording_grant(self, operation_id: str = "safe-point") -> None:
        state = adapter._read_json(adapter._ledger(self.repo, "run-local-123"))
        state["recording_grant"] = {
            "harness_run_id": "harness-local-123",
            "operation_id": operation_id,
        }
        adapter._atomic_json(adapter._ledger(self.repo, "run-local-123"), state)
    ...
    def test_verify_blocks_existing_failed_cq_before_local_verification(self) -> None:
        ...
        self.assertEqual(result["next_action"], "recording-health-and-retry-verify")
    ...
    def test_verify_recovers_failed_cq_with_fresh_grant_then_persists(self) -> None:
        self._mark_recording_ready()
        verification = self._bind_required_cq()
        ...
        self.assertTrue(any(path.endswith("/validate") for path in calls))
        self.assertNotIn(
            "cq_recovery_required",
            adapter._read_json(adapter._ledger(self.repo, "run-local-123")),
        )
    ...
    def test_finish_requires_every_ontology_before_completion(self) -> None:
        ...
        with (
            ...
            with self.assertRaisesRegex(adapter.AdapterError, "verification_not_passed"):
                adapter.finish(self.repo, self.run_dir, self.config, "safe-point")
        request.assert_not_called()
        self._mark_recording_ready()
        with (
            mock.patch.object(adapter, "load_config", return_value=({}, {})),
            ...
            result = adapter.finish(self.repo, self.run_dir, self.config, "safe-point")
        self.assertEqual(result["status"], "ok")
        self.assertTrue(
            any(":complete" in call.args[2] for call in completed_request.call_args_list)
        )

    def test_finish_requires_current_safe_point_when_harness_is_unbound(self) -> None:
        adapter._atomic_json(
            adapter._ledger(self.repo, "run-local-123"),
            {
                "schema_version": 1,
                "build_session_id": "session-1",
                "recording_unavailable": {"operation_id": "earlier-safe-point"},
            },
        )
        run = {"run_id": "run-local-123", "ontologies": [{"ontology_id": "o"}]}
        with (
            mock.patch.object(adapter, "load_config", return_value=({}, {})),
            mock.patch.object(adapter.smd, "_load_run", return_value=run),
        ):
            with self.assertRaisesRegex(adapter.AdapterError, "recording_health_required"):
                adapter.finish(self.repo, self.run_dir, self.config, "finish-safe-point")

    def test_finish_allows_current_recording_unavailable_without_harness_and_is_idempotent(
        self,
    ) -> None:
        operation_id = "finish-safe-point"
        adapter._atomic_json(
            adapter._ledger(self.repo, "run-local-123"),
            {
                "schema_version": 1,
                "build_session_id": "session-1",
                "recording_unavailable_authorization": {
                    "operation_id": operation_id,
        ...
        with (
            ...
            mock.patch.object(adapter, "_request", side_effect=request),
            mock.patch.object(adapter.subprocess, "run") as finalize,
        ):
            first = adapter.finish(self.repo, self.run_dir, self.config, operation_id)
            state = adapter._read_json(adapter._ledger(self.repo, "run-local-123"))
            state["recording_unavailable_authorization"] = {
                "operation_id": operation_id,
                "reason": "explicit retry authorization",
            }
            adapter._atomic_json(adapter._ledger(self.repo, "run-local-123"), state)
            second = adapter.finish(self.repo, self.run_dir, self.config, operation_id)

        self.assertEqual(first["status"], "ok")
        self.assertEqual(first["next_action"], "done-recording-incomplete")
        self.assertEqual(first["findings"], [{"code": "recording_unavailable"}])
        self.assertNotIn("harness_run_id", first["references"])
        self.assertEqual(second["status"], "ok")
        self.assertEqual(
            [payload["client_request_id"] for payload in completed_payloads],
            [adapter._attempt_identity("run-local-123", "session", "complete")] * 2,
        )
        self.assertTrue(
            all(
                payload["summary"] == "Local modeling verification passed; recording unavailable"
        ...
        self.assertEqual(state["recording_unavailable"], {"operation_id": operation_id})

    def test_health_consumes_bounded_harness_result_and_cancel_blocks_unreconciled_apply(
        self,
    ) -> None:
        adapter._atomic_json(
            adapter._ledger(self.repo, "run-local-123"),
            {"schema_version": 1, "build_session_id": "session-1", "attempts": {}},
        )
        completed = mock.Mock(returncode=0, stdout='{"healthy": true}')
        with mock.patch.object(adapter.subprocess, "run", return_value=completed):
            result = adapter.recording_health(
                self.repo, self.run_dir, "operation-health-001", "run-local-123"
            )
        self.assertEqual(result["status"], "ok")
        state = adapter._read_json(adapter._ledger(self.repo, "run-local-123"))
        state["attempts"] = {"batch": {"mode": "apply_atomic", "reconciled": False}}
        adapter._atomic_json(adapter._ledger(self.repo, "run-local-123"), state)
    ...
    def test_apply_timeout_keeps_original_identity_for_reconciliation(self) -> None:
        ...
        with (
            ...
            result = adapter.apply_next(self.repo, self.run_dir, self.config, "o", "safe-point")
