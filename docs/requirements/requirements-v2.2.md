...
- L3 的业务切片固定为 v2.1 M1/M6 已使用的完整 Dify Workflow-as-Tool `C -> B -> A`
  调用影响链，以便与既有单 Agent 证据进行同源比较；L3 使用全新 Session、运行目录、Project、
  Ontology 和 Build Session，且 M1–M6 的答案型本体、Batch、查询答案、运行记录和隐藏验收合同
  继续属于 `tester-only`，不得进入团队可见输入；
...
L3 的质量验收和评测设施范围仍需逐项细化，不能因合并 L2 而自动扩大。
