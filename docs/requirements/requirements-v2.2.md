...
- L3 使用真实业务切片验证三 Agent 架构能否完成一次端到端建模闭环；L3 不设置单 Agent
  对照组、不做提升幅度比较，也不宣称该架构已经证明优于单 Agent；
- L3 的业务切片固定为 v2.1 M1/M6 已使用的完整 Dify Workflow-as-Tool `C -> B -> A`
  调用影响链，以控制业务范围并复用已经冻结的来源资料和业务问题；L3 使用全新 Session、运行
  目录、Project、Ontology 和 Build Session，且 M1–M6 的答案型本体、Batch、查询答案、运行
  记录和隐藏验收合同继续属于 `tester-only`，不得进入团队可见输入；
- L3 当前不自动扩展到 v2.1 M7 的完整 Workflow 编排与类型化变量流转模块；是否增加该模块必须
  作为新的范围决定另行确认；
...
- 只验收 L3 真实运行中自然产生的错误路由；某类错误未自然出现不阻断 L3，也不为此增加独立
  错误矩阵、故障注入框架、验收程序或单独 L2 场景；
...
- 测试侧在运行前冻结业务缺口答案并保存在 `tester-only`，本体建模团队不可见；
- 只有建模协调 Agent 基于可见资料提出一个会实质改变模型或业务查询的问题后，交付 Agent 才
  原样释放对应的冻结答案；未被团队识别和询问的缺口不得由交付 Agent 主动提示；
- 问题继续遵循一次一个的交互合同。交付 Agent 只做问题与答案的机械匹配和转发，不补充建模
  建议、隐藏验收条件或答案型结构；
- 若冻结答案本身为业务方无法确认，必须原样返回“无法确认”，由团队决定如何表达显式未知；
- 每个问题、原始回答、恢复的协调 Session 和最终模型影响均保留可复核证据。
