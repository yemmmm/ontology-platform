...
本设计落实 `docs/requirements/requirements-v1.0.md` 的 R-011。首次交付时，R-008 尚未实现；该事实只用于解释
...
- `docs/reference/api.md` 同时描述当前接口和已经移除的 Version、Proposal、Catalog、Neo4j Entity 等接口。
- `docs/reference/mcp.md` 仍列出已经从 `register_all` 删除的 governance、catalog、entity、fact 工具。
...
- `docs/reference/api.md` 只列出真实注册的 HTTP 操作，并解释当前主要契约和已知缺口。
- `docs/reference/mcp.md` 只列出真实注册工具；完整清单由 runtime registry 生成。
- 修正会影响开发判断的 `AGENTS.md`、`docs/guides/platform-guide.md`、`docs/guides/ui.md` 和
  `docs/architecture/overview.md` 当前态矛盾。
...
- `docs/guides/platform-guide.md`：修正启动、健康检查、存储和认证说明；删除对旧治理接口的当前态描述。
- `docs/guides/ui.md`：修正默认 API base、依赖健康和 Agent Test 当前边界。
- `docs/architecture/overview.md`：把 PostgreSQL + RDF Dataset/Oxigraph 标为当前权威存储；Neo4j 只在
  明确的历史背景中出现，不能作为当前写入路径。
- `docs/requirements/requirements-v1.0.md`：修正过期的“当前实现基线”行，并在首次交付完成后更新 R-011 状态；
...
2. `docs/reference/api.md` 生成清单与 115 个当前 HTTP 操作完全一致，且没有旧接口被描述为当前能力。
3. `docs/reference/mcp.md` 生成清单与 55 个当前 MCP 工具完全一致，且无第二份手工全量清单。
