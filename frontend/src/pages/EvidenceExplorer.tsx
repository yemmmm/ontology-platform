...
export function EvidenceExplorer({
  ...
  const hydrate = useCallback(async (items: Evidence[]) => {
    ...
    setContexts(items.map((item) => {
      ...
      return {
        ...
        chunk,
        traceWarning: hashMismatch ? "The stored evidence hash differs from the current evidence chunk hash." : null,
      };
    }));
  }, [request]);
  ...
  return (
    <Space direction="vertical" size={compact ? 8 : 16} style={{ width: "100%" }}>
      {!compact && <div className="topBar"><div><span className="eyebrow">Traceability / artifact truth</span><h1>Evidence Explorer</h1><div className="crumbTrail">Artifact, conversation and user-statement evidence</div></div><Button icon={<RefreshCw size={15} />} onClick={() => void load()} loading={loading}>Refresh</Button></div>}
      ...
      {!compact && <Card title="Evidence lookup">
        <Space wrap style={{ width: "100%" }}>
          ...
          <Select value={selectedDocumentId} onChange={setSelectedDocumentId} style={{ width: 240 }} options={[{ value: "all", label: "All evidence artifacts" }, ...documents.map((item) => ({ value: item.id, label: item.filename }))]} />
        </Space>
      </Card>}
      {loading && <Spin tip="Loading evidence and evidence chunks…" />}
      {unresolvedIds.length > 0 && !proposalId && !providedEvidence && (
        <Alert type="warning" showIcon message="Evidence IDs cannot be resolved by the current backend API." description={`Unresolved: ${unresolvedIds.join(", ")}. The backend exposes evidence through proposal item sources only; Fact Claims do not include proposal/item linkage. No artifact location has been fabricated.`} />
      )}
      {(artifactId || documentId) && !proposalId && !providedEvidence && (
        <Alert type="warning" showIcon message="Artifact-wide evidence lookup is not available." description="The backend can list artifact proposals and proposal-item evidence, but it cannot list evidence records for an artifact directly. Supply a proposal ID and item key to resolve exact sources." />
      )}
